...
const MemberTable = ({ members, scheduleMap = {}, onUpdate, onSetSchedule }) => {
  ...
  const handleDelete = async (member) => {
    if (!confirm(`Delete ${member.name}? Their logs will be preserved.`)) return
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-slate-100 dark:border-slate-700/60">
              {['Member', 'Role', 'Department', 'Status', 'Shift Schedule', user?.role === 'owner' ? 'Actions' : '']
                .filter(Boolean)
                .map((h) => (
                  <th
                    key={h}
            ...
            {members.map((member) => {
              ...
              return (
                ...
                >
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <button
                          ...
                          className="font-medium text-slate-800 dark:text-slate-200 hover:text-brand-500 dark:hover:text-brand-400 transition-colors text-left"
                        >
                          {member.name}
                        </button>
                  ...
                  {/* Department */}
                  <td className="px-5 py-4 text-sm text-slate-500 dark:text-slate-400" style={tdStyle}>
                    {member.department || '—'}
                  </td>
                      ...
                      >
                        ...
                        <div className="text-left">
                          ...
                          <p className="text-[10px] text-slate-400 mt-0.5">
                            {DAY_LABELS.filter((_, i) => schedule.workDays.includes(i)).join(' · ')}
                          </p>
                        </div>
                      </button>
                    ) : (
