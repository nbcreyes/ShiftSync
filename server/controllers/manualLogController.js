...
const submitRequest = async (req, res) => {
  try {
    ...
    if (!date || !timeIn || !timeOut || !reason) {
      return res.status(400).json({ message: 'Date, time in, time out, and reason are required' })
    }
    ...
    if (existingLog) {
      return res.status(409).json({ message: 'A time log already exists for this date' })
    }
    ...
    // ── Notify all admins + owner in the workspace ──
...
const approveRequest = async (req, res) => {
  try {
    ...
    // Check again if a log was created in the meantime
    const existingLog = await TimeLog.findOne({
      tenantId: req.tenantId,
    ...
    const totalMins = (new Date(request.timeOut) - new Date(request.timeIn)) / 60000
