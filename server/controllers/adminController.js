...
const updateRole = async (req, res) => {
  try {
    ...
    if (user.role === "owner") {
      return res
        .status(403)
        .json({ message: "Cannot change the role of the workspace owner" });
    }
...
const deactivateUser = async (req, res) => {
  try {
    ...
    if (user.role === "owner") {
      return res
        .status(403)
        .json({ message: "Cannot deactivate the workspace owner" });
    }
...
const deleteUser = async (req, res) => {
  try {
    ...
    if (user.role === "owner") {
      return res
        .status(403)
        .json({ message: "Cannot delete the workspace owner" });
    }
    ...
    return res
      .status(200)
      .json({ message: "User deleted. Their logs have been preserved." });
  } catch (error) {
...
const getLiveBoard = async (req, res) => {
  try {
    ...
    const openBreaks = await Break.find({
      tenantId: req.tenantId,
      breakEnd: null,
    });
    ...
    const board = logs.map((log) => {
      ...
      if (!log.timeOut) {
        status = openBreakLogIds.has(log._id.toString())
          ? "on-break"
          : "working";
      }
      return {
...
const exportCSV = async (req, res) => {
  try {
    ...
    const header =
      "Name,Email,Department,Date,Time In,Time Out,Break Mins,Hours Worked,Overtime,Status,Note";
    const csv = [header, ...rows].join("\n");

    res.setHeader("Content-Type", "text/csv");
    res.setHeader(
      "Content-Disposition",
      "attachment; filename=team-timelog.csv",
    );
...
const getSummaryReport = async (req, res) => {
  try {
    ...
    if (!startDate || !endDate || !timezone) {
      return res
        .status(400)
        .json({ message: "startDate, endDate, and timezone are required" });
    }
    ...
    const scheduleMap = {};
    schedules.forEach((s) => {
      scheduleMap[s.userId.toString()] = s;
    });
...
const getDepartmentReport = async (req, res) => {
  try {
    ...
    const userDeptMap = {};
    users.forEach((u) => {
      userDeptMap[u._id.toString()] = u.department || "Unassigned";
    });
...
const getPendingCounts = async (req, res) => {
  try {
    const [pendingLeave, pendingManualLogs] = await Promise.all([
      LeaveRequest.countDocuments({
        tenantId: req.tenantId,
        status: "pending",
      }),
      ManualLogRequest.countDocuments({
        tenantId: req.tenantId,
        status: "pending",
      }),
    ]);

    return res.status(200).json({ pendingLeave, pendingManualLogs });
  } catch (error) {
    console.error("GetPendingCounts error:", error.message);
    return res.status(500).json({ message: "Server error" });
  }
};
...
const getMemberDetail = async (req, res) => {
  try {
    ...
    const user = await User.findOne({ _id: id, tenantId: req.tenantId }).select(
      "-passwordHash -refreshToken -resetPasswordToken -resetPasswordExpires",
    );
    if (!user) return res.status(404).json({ message: "Member not found" });

    const year = new Date().getFullYear();
    const [logs, leaves, shift, flags, balance] = await Promise.all([
      TimeLog.find({ tenantId: req.tenantId, userId: id })
        .sort({ date: -1 })
        .limit(30),
      LeaveRequest.find({ tenantId: req.tenantId, userId: id })
        .sort({ createdAt: -1 })
      ...
      TimeLog.find({
        ...
        .sort({ date: -1 })
        .limit(20),
      LeaveBalance.findOne({ tenantId: req.tenantId, userId: id, year }),
    ]);

    // Summary stats
    const completedLogs = logs.filter((l) => l.timeOut);
    const totalWorkedMins = completedLogs.reduce((sum, l) => {
      const mins =
        (new Date(l.timeOut) - new Date(l.timeIn)) / 60000 -
        (l.totalBreakMins || 0);
    ...
    const stats = {
      ...
      totalWorkedHours: parseFloat((totalWorkedMins / 60).toFixed(1)),
      avgWorkedHours: completedLogs.length
        ? parseFloat((totalWorkedMins / 60 / completedLogs.length).toFixed(1))
        : 0,
      overtimeDays: logs.filter((l) => l.overtime).length,
    ...
    return res
      .status(200)
      .json({ user, logs, leaves, shift, flags, stats, balance });
  } catch (error) {
...
// ─── Flag Trends Report ───────────────────────────────────────────────────────
const getFlagTrends = async (req, res) => {
  try {
    const { startDate, endDate, timezone } = req.query
    if (!startDate || !endDate || !timezone) {
    ...
    if (schedules.length === 0) return res.status(200).json({ buckets: [], users: [] })
    ...
    const logMap = {}
    logs.forEach((log) => {
      logMap[`${log.userId.toString()}__${log.date}`] = log
    })
    ...
    // Bucket dates into weeks (ISO week label = Monday date)
    ...
    for (const schedule of schedules) {
      const uid = schedule.userId._id.toString()
      for (const date of dates) {
        ...
        weekSet.add(weekLabel)

        if (!userMap[uid].weeks[weekLabel]) {
          userMap[uid].weeks[weekLabel] = { late: 0, absent: 0 }
        }

        const log = logMap[`${uid}__${date}`]
        if (!log) {
          userMap[uid].weeks[weekLabel].absent++
        } else if (log.timeIn) {
          ...
          const scheduledMins = toMinutes(schedule.startTime)
          if (actualMins - scheduledMins > schedule.lateTolerance) {
            userMap[uid].weeks[weekLabel].late++
          }
        }
      }
    }
    ...
    return res.status(200).json({ weeks: sortedWeeks, users })
  } catch (error) {
    console.error('GetFlagTrends error:', error.message)
    return res.status(500).json({ message: 'Server error' })
  }
}
