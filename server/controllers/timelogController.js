...
const calculateTotalBreakMins = async (logId) => {
  ...
  return breaks.reduce((total, b) => {
    if (b.breakStart && b.breakEnd) {
      const mins = (new Date(b.breakEnd) - new Date(b.breakStart)) / 60000
      return total + mins
    }
    return total
  }, 0)
...
const editLog = async (req, res) => {
  try {
    ...
    if (log.status !== 'remarked') {
      return res.status(403).json({ message: 'Log can only be edited when it has been remarked' })
    }
...
const exportCSV = async (req, res) => {
  try {
    ...
    const header = 'Date,Time In,Time Out,Break Mins,Hours Worked,Overtime,Status,Note'
